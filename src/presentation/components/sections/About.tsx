...
export const About: React.FC<AboutProps> = ({ data, experiences, projects, certifications }) => {
  ...
  return (
    <section id="about" className="py-20 bg-tokyo-bg-dark">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          {stats.map((stat, index) => (
            <ScrollReveal key={index} delay={index * 0.1}>
              <Card className="text-center hover:border-tokyo-blue transition-all duration-300 hover:shadow-lg hover:shadow-tokyo-blue/20">
                <CardContent className="pt-6">
                  <stat.icon className="w-12 h-12 mx-auto mb-4 text-tokyo-blue" />
                  <h3 className="text-3xl font-bold text-tokyo-fg mb-2">{stat.value}+</h3>
                  <p className="text-tokyo-comment">{stat.label}</p>
                </CardContent>
              </Card>
            </ScrollReveal>
          ))}
              ...
              <div className="space-y-4">
                {data.latestPositions.map((position, index) => (
                  <FadeIn key={index} delay={index * 0.1}>
                    <div className="p-4 bg-tokyo-bg-light rounded-lg border border-tokyo-black hover:border-tokyo-blue transition-all">
